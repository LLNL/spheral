#ATS:test(SELF, "--graphics False --nx1 10 --nx2 10 --testDim 1d", label="CSPH sum mass density test -- 1D (serial)")
#ATS:test(SELF, "--graphics False --nx1 10 --nx2 10 --testDim 2d", label="CSPH sum mass density test -- 2D (serial)")
#ATS:test(SELF, "--graphics False --nx1 10 --nx2 10 --testDim 3d", label="CSPH sum mass density test -- 3D (serial)")
#-------------------------------------------------------------------------------
# Unit test of the CSPH sum density algorithm.
#-------------------------------------------------------------------------------
from Spheral import *
from SpheralTestUtilities import *

title("CSPH sum density tests")
...
#-------------------------------------------------------------------------------
erryCSPH =  ScalarField("CSPH mass density error", nodes1)
for i in xrange(nodes1.numInternalNodes):
    erryCSPH[i] =   rho[i] - yans[i]
maxyCSPHerror = erryCSPH.max()
print "Maximum error: CSPH = %g" % (maxyCSPHerror)
...
if graphics:
    ...
    ansdata = Gnuplot.Data(xans, yans.internalValues(),
                           ...
                           title = "Answer",
                           inline = True)
    CSPHdata = Gnuplot.Data(xans, rho.internalValues(),
                            with_ = "points",
                            title = "CSPH",
                            inline = True)
    ...
    if testDim == "2d":
        ...
        dumper = SpheralVoronoiSiloDump("testCSPHSumDensity_%s_2d" % testCase,
                                        listOfFields = [rho, yans, erryCSPH],
...
#-------------------------------------------------------------------------------
if maxyCSPHerror > tolerance:
    raise ValueError, "CSPH error out of bounds: %g > %g" % (maxyCSPHerror, tolerance)
