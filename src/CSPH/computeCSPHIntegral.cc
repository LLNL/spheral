...
computeCSPHIntegral(const ConnectivityMap<Dimension>& connectivityMap,
  ...
  if(dim == mydim){//Lowest level perform 1d normal Romberg
    ...
    computeCSPHEvaluation(connectivityMap,W,weight,position,H,nodeListi, i, rmin,
                          true, Wai, gradWai);
    computeCSPHEvaluation(connectivityMap,W,weight,position,H,nodeListj, j, rmin,
                          true, Waj, gradWaj);
 
    integ[0][0]=Wai*gradWaj*wi*wj;
    integ2[0][0]=Waj*gradWai*wi*wj;
    computeCSPHEvaluation(connectivityMap,W,weight,position,H,nodeListi, i, rmax,
                        true, Wai, gradWai);
    computeCSPHEvaluation(connectivityMap,W,weight,position,H,nodeListj, j, rmax,
                        true, Waj, gradWaj);
    integ[0][0]+=Wai*gradWaj*wi*wj;
    integ2[0][0]+=Waj*gradWai*wi*wj;
  }else{
    ...
    integ[0][0]=computeCSPHIntegral(connectivityMap,W,weight,position,H,nodeListi, i, nodeListj, j, mydim+1, order, temprmin, temprmax).first;
    integ2[0][0]=computeCSPHIntegral(connectivityMap,W,weight,position,H,nodeListi, i, nodeListj, j, mydim+1, order, temprmin, temprmax).second;
    temprmax=rmax;
    ...
    for(int mm=0; mm <= nn; mm++){
       if(mm==0){
          ...
          for(int k=1; k<= pow(2,nn-1); k++){
              ...
              if(dim == mydim){
                 computeCSPHEvaluation(connectivityMap,W,weight,position,H,nodeListi, i, reval,
                           true, Wai, gradWai);
                 computeCSPHEvaluation(connectivityMap,W,weight,position,H,nodeListj, j, reval,
                           true, Waj, gradWaj);
                ...
                integ[nn][mm]+=hn*computeCSPHIntegral(connectivityMap,W,weight,position,H,nodeListi, i, nodeListj, j, mydim+1, order, temprmin, temprmax).first;
                integ2[nn][mm]+=hn*computeCSPHIntegral(connectivityMap,W,weight,position,H,nodeListi, i, nodeListj, j, mydim+1, order, temprmin, temprmax).second;
              }
