...
CSPHSpecificThermalEnergyPolicy<Dimension>::
CSPHSpecificThermalEnergyPolicy(const DataBase<Dimension>& dataBase,
...
template<typename Dimension>
CSPHSpecificThermalEnergyPolicy<Dimension>::
~CSPHSpecificThermalEnergyPolicy() {
}
...
operator==(const UpdatePolicyBase<Dimension>& rhs) const {
  ...
  const CSPHSpecificThermalEnergyPolicy<Dimension>* rhsPtr = dynamic_cast<const CSPHSpecificThermalEnergyPolicy<Dimension>*>(&rhs);
  if (rhsPtr == 0) {
    return false;
  } else {
    return true;
