//---------------------------------Spheral++----------------------------------//
// HostCodeBoundary -- Take a snapshot of the state of a set of nodes on 
...
// Modified by:
//----------------------------------------------------------------------------//
#ifndef HostCodeBoundary_HH
#define HostCodeBoundary_HH
...
class HostCodeBoundary: public Boundary<Dimension> {
...
public:
  ...
  // Accessor methods.
  void updateNodeIDs(const vector<int>& nodeIDs);
...
private:
  ...
  HostCodeBoundary();
  HostCodeBoundary(HostCodeBoundary&);
};

}

#include "HostCodeBoundaryInline.hh"

#else

// Forward declaration.
namespace Spheral {
  template<typename Dimension> class HostCodeBoundary;
}
