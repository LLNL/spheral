...
#pragma omp for
    for (auto kk = 0u; kk < npairs; ++kk) {
      ...
      //--------------------------------------------------------
      //const auto voli = (isMFM ? volumei : volumej);
      //const auto volj = (isMFM ? volumej : volumei);

      psii = (isMFM ? voli : volj)*Wi;
      psij = (isMFM ? volj : voli)*Wj;
      gradPsii = (isMFM ? voli : volj) * Mi.Transpose()*gradWi;
      gradPsij = (isMFM ? volj : voli) * Mj.Transpose()*gradWj;

      const auto AstarMFM = voli*gradPsii + volj*gradPsij;
      const auto Ai = (isMFM ? 0.5*AstarMFM : voli*gradPsii);
      const auto Aj = (isMFM ? 0.5*AstarMFM : volj*gradPsij);
      //const auto Ai = ;
      //const auto Aj = 0.5*AstarMFM;
      // acceleration
      //------------------------------------------------------
...
#pragma omp for
    for (auto kk = 0u; kk < npairs; ++kk) {
      ...
      const auto gradPsii = (isMFM ? voli : volj)*gradWi;
      const auto gradPsij = (isMFM ? volj : voli)*gradWj;
