...
namespace FractalSpace
{
  ...
  void sor(Group& group, Fractal& fractal,vector <Point*>& list_edge,const int& dir)
  {
    ...
    for(int go_around=0;go_around<2;go_around++)
      {
	for(n=0;n< maxits_real;++n)
	  {
	    ...
	    converged = anorm < fractal.get_epsilon_sor()*anormf && anorm2 < fractal.get_epsilon_sor()*anorm2f;
	    if(converged || go_around == 1)
	      {
		FileSor.precision(4);
		if(go_around == 1) FileSor << " try converging again " << endl;
		FileSor << n << "\t " 
			<< n_points << "\t " 
			<< scientific
			<< anorm << "\t " 
