	...
	for(int tr=0;tr<2;tr++)
	  {
		...
		//		cout << " counts Z " << FractalRank << " " << nz << " " << snumbersz[nz] << endl;
	      }
		...
		fprintf(PFFM," target a %d \t %10.2E \t %10.2E \t %d \n",FRZ,target,pztotal,lowerz[FRZ]);
		if(FRZ > 0)
		...
		for(int FRZ=0;FRZ<FractalNodes2;FRZ++)
		  {
		    double mult=1.0+3.0/static_cast<double>(upperz[FRZ]-lowerz[FRZ]);
		    for(int nz=lowerz[FRZ];nz<upperz[FRZ];nz++)
		      numbersz0[nz]*=mult;
		  }
	      }
	  }
		...
		for(int ny=0;ny<real_length;ny++)
		  {
		    ...
		    //		    cout << " counts Y " << FractalRank << " " << FRZ << " " << ny << " " << snumbersy[ny] << endl;
		  }		
		...
		for(int FRY=0;FRY<FractalNodes1;FRY++)
		  {
		    ...
		    fprintf(PFFM," target b %d \t %10.2E \t %10.2E \t %d \n",FRY,target,pytotal,lowery[FRZ][FRY]);
		    if(FRY > 0)
		...
		if(tr == 0)
		  {
		    for(int FRY=0;FRY<FractalNodes1;FRY++)
		      {
			double mult=1.0+3.0/static_cast<double>(uppery[FRZ][FRY]-lowery[FRZ][FRY]);
			for(int ny=lowery[FRZ][FRY];ny<uppery[FRZ][FRY];ny++)
			  numbersy0[ny]*=mult;
		      }
		  }
	      }
		...
		for(int tr=0;tr<2;tr++)
		  {
			...
			//			cout << " counts X " << FractalRank << " " << FRZ << " " << FRY << " " << nx << " " << snumbersx[nx] << endl;
		      }
			...
			fprintf(PFFM," target c %d \t %10.2E \t %10.2E \t %d \n",FRX,target,pxtotal,lowerx[FRZ][FRY][FRX]);
			if(FRX > 0)
			...
			for(int FRX=0;FRX<FractalNodes0;FRX++)
			  {
			    double mult=1.0+3.0/static_cast<double>(upperx[FRZ][FRY][FRX]-lowerx[FRZ][FRY][FRX]);
			    for(int nx=lowerx[FRZ][FRY][FRX];nx<upperx[FRZ][FRY][FRX];nx++)
			      numbersx0[nx]*=mult;
			  }
		      }
		  }
