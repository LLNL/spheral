...
namespace FractalSpace
{
  bool test_group(Group& group)
  {
	...
	for(int ni=0;ni<3;ni++)
	  {
	    ...
	    if(ud[ni1] != 0) sumn++;
	    badd1= badd1 ||
	      (ud[ni0] != 0 && ud[ni0]->get_point_ud(ni1) != &point ) ||
	      (ud[ni1] != 0 && ud[ni1]->get_point_ud(ni0) != &point );
	    ni0+=2;
	    ni1+=2;
	  }
	int rpx=rp % 3;
	...
	bool badd4=point.get_inside() && sumn < 6;
	bool badd5= (sumr == 0 && point.get_point_pointer_t() == 0) ||(sumr != 0 && point.get_point_pointer_t() != 0);
	...
	if(badd)
	  {
	    //	    cout << "badd " << badd1 << badd2 << badd3 << badd4 << badd5 << badd6 << "\n";
	    point.dump();
	    assert(!badd);
	  }
      }
