...
namespace FractalSpace
{
  ...
  {
    if(fractal.get_debug()) cout << "enter assign density " << &group << " " << group.get_level() << endl;
    //--------------------------------------------------------------------------------------------------------------------------------
  ...
  //--------------------------------------------------------------------------------------------------------------------------------
	...
	//      cout << " scaling " << scaling << " " << group.get_level() << endl;
	for(vector<Point*>::const_iterator point_itr=group.list_points.begin();point_itr !=group.list_points.end();++point_itr)
	  {
	    //--------------------------------------------------------------------------------------------------------------------------------
	    // use scaling from mass per point to real density
	    //--------------------------------------------------------------------------------------------------------------------------------
	    if((*point_itr)->get_inside()) (*point_itr)->scale_density_point(scaling);
	  }
      }
    double d_0=fractal.get_density_0();
    //  cout << "density zero " << d_0 << endl;
    //--------------------------------------------------------------------------------------------------------------------------------
    // subtract mean density
    //--------------------------------------------------------------------------------------------------------------------------------
    if(group.get_set_dens() && d_0 != 0.0)
      group.subtract_density(d_0);
    if(fractal.get_debug()) cout << "leaving assign density" << endl;
    //
  }
}
