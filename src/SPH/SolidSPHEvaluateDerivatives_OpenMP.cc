...
#pragma vector always
          for (auto jItr = connectivity.begin();
            ...
            if (nodeListi == nodeListj) rhoSumi += mj*Wi;
            ...
            auto Peffi = Pi;
            auto Peffj = Pj;
            if (mNegativePressureInDamage) {
              if (Pi < 0.0 or Pj < 0.0) {
                // const auto Pmin = min(0.0, damage(nodeListi, i).Trace() < damage(nodeListj, j).Trace() ? Pi : Pj);
                const auto Pmin = std::max(Pi, Pj);
                if (Pi < 0.0) Peffi = fDeffij*Peffi + (1.0 - fDeffij)*Pmin;
                if (Pj < 0.0) Peffj = fDeffij*Peffj + (1.0 - fDeffij)*Pmin;
              }
            } else {
              if (Pi < 0.0) Peffi *= fDeffij;
              if (Pj < 0.0) Peffj *= fDeffij;
            }
            ...
            if (freeParticle) DvDti += mj*deltaDvDt;
            ...
            if (compatibleEnergy) pairAccelerationsi.push_back( mj*deltaDvDt);

            // Velocity gradient.
            DvDxi -= mj*deltaDvDxi;
            if (sameMatij) localDvDxi -= mj*deltaDvDxi;
            ...
            if (sameMatij) localMi -= mj*rij.dyad(gradWGi);
          }
        }
      }
