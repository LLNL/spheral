...
//------------------------------------------------------------------------------
// On problem start up, we set the Spherical flag on the DataBase.
//------------------------------------------------------------------------------
void
SPHHydroBaseSpherical::
initializeProblemStartup(DataBase<Dim<1>>& dataBase) {
  dataBase.isSpherical = true;
  SPHHydroBase<Dim<1>>::initializeProblemStartup(dataBase);
}
...
//   // Now convert back to true masses and mass densities.  We also apply the Spherical
...
evaluateDerivatives(const Dim<1>::Scalar /*time*/,
  ...
  const auto& W = this->sphericalKernel();
...
#pragma omp for
    for (auto kk = 0u; kk < npairs; ++kk) {
      ...
      std::tie(Wi, gWi) = W.kernelAndGradValue(etaj, etai, Hdeti);
      const auto& WQi = Wi;
      const auto& gWQi = gWi;
