    ...
    for (auto kk = 0u; kk < npairs; ++kk) {
     ...
     // dwdhj = -(Dimension::nDim*Wj + etaMagj*gWj)*Dimension::rootnu(Hdetj);
      ...
      if(!sameMatij){
        ...
        vij = muij/max(rij2,tiny)*rij; 
        const auto surfaceDecouplingi = (muij > 0.0 && Pi < 0.0);
        const auto surfaceDecouplingj = (muij > 0.0 && Pj < 0.0);
        const auto kappai = (surfaceDecouplingi ? 0.0 : max(0.0, min(2.0, 2.0*(Kj*voli*yi*gWj)/(Ki*volj*yj*gWi+Kj*voli*yi*gWj))));
        const auto kappaj = (surfaceDecouplingj ? 0.0 : max(0.0, min(2.0, 2.0-kappai)));

        deltaDvDxi = kappai*fDeffi*vij.dyad(gradWi);
        deltaDvDxj = kappaj*fDeffj*vij.dyad(gradWj);
      }
...
#pragma omp parallel for
    for (auto i = 0u; i < ni; ++i) {
     ...
     //   CHECK(XSPHWeightSumi >= 0.0);
      ...
      const auto deformation = DvDxi.Symmetric();
      const auto spin = DvDxi.SkewSymmetric();
