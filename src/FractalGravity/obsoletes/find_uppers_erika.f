      subroutine find_uppers_erika(adj,periodic,grid_multiply,
     ...
     >  hoc_high,next_high_points,high_points_maxx,points_maxx)
c     
      implicit none
c     
      include 'maxx.inc'
c     
      ...
      integer point_up_z(points_maxx),p_x,p_y,p_z,m
...
c     
      ...
      logical periodic,problem_x,problem_y,problem_z,dist_p
c
        ...
        do p_z=1,3
          do p_y=1,3
            do p_x=1,3
              ...
              must_exist(1,p)=p_x .lt. 3
              must_exist(2,p)=p_y .lt. 3
              must_exist(3,p)=p_z .lt. 3
            end do
          end do
        end do
      end if
...
c     
        ...
        if(h_point .gt. 0) then
          ...
          if(p_x .lt. 0) then
            call slow_search(adj,periodic,grid_multiply,
     >        hoc_daughter_points,next_daughter_points,pos_point_x,
     >        pos_point_y,pos_point_z,point,delta,0,0,p_x,
     >        high_points_maxx,points_maxx)
            write(49,*)'xa ',high_point,point,h_point,p_p,
     >        pos_high_x(m),p_x
          end if
          if(p_x .lt. 0) then
            call emergency_search(periodic,grid_multiply,
     >        hoc_high,next_high_points,
     >        hoc_daughter_points,next_daughter_points,pos_point_x,
     >        pos_point_y,pos_point_z,point,delta,0,0,delta,p_x,
     >        high_points_maxx,points_maxx)
            write(49,*)'xb ',high_point,point,h_point,p_p,
     >        pos_high_x(m),p_x
          end if
        end if
c     
        ...
        if(h_point .gt. 0) then
          ...
          if(p_y .lt. 0) then
            call slow_search(adj,periodic,grid_multiply,
     >        hoc_daughter_points,next_daughter_points,pos_point_x,
     >        pos_point_y,pos_point_z,point,0,delta,0,p_y,
     >        high_points_maxx,points_maxx)
            write(49,*)'ya ',high_point,point,h_point,p_p,
     >        pos_high_y(m),p_y
          end if
          if(p_y .lt. 0) then
            call emergency_search(periodic,grid_multiply,
     ...
     >        high_points_maxx,points_maxx)
            write(49,*)'yb ',high_point,point,h_point,p_p,
     >        pos_high_y(m),p_y
          end if
        end if
c     
        ...
        if(h_point .gt. 0) then
          ...
          if(p_z .lt. 0) then
            call slow_search(adj,periodic,grid_multiply,
     >        hoc_daughter_points,next_daughter_points,pos_point_x,
     >        pos_point_y,pos_point_z,point,0,0,delta,p_z,
     >        high_points_maxx,points_maxx)
            write(49,*)'za ',high_point,point,h_point,p_p,
     >        pos_high_z(m),p_z
          end if
          if(p_z .lt. 0) then
            call emergency_search(periodic,grid_multiply,
     ...
     >        high_points_maxx,points_maxx)
            write(49,*)'zb ',high_point,point,h_point,p_p,
     >        pos_high_z(m),p_z
          end if
        end if
c     
...
c     
        if(badd_z) then
          write(49,*)'pos z u ',badd_z,delta,
     >      pos_point_x(p_z),pos_point_x(point),
     >      pos_point_y(p_z),pos_point_y(point),
     >      pos_point_z(p_z),pos_point_z(point)
        end if
c     
        problem_x=must_exist(1,m) .and. point_up_x(point) .le. 0
        problem_y=must_exist(2,m) .and. point_up_y(point) .le. 0
        problem_z=must_exist(3,m) .and. point_up_z(point) .le. 0
c     
        if(problem_x .or. problem_y .or. problem_z) then
          write(59,*)'prob xyz ',high_point,m,point,point_up_x(point),
     >      point_up_y(point),point_up_z(point),
