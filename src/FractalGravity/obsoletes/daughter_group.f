...
c     
      ...
      integer n,i,j
c      integer hoc_backwards,next_backwards(maxx)
c     
...
c     
        ...
        point_0=point
c     
        call find_real_empties(high_group,high_point,
...
c     
c     call find_empties(high_point,empty,
...
c     
      call cpu_time(t_2)
c     
c     check_remove=next_high_points(hoc_high_points(high_group)) .gt. 0
c     
c      check_remove=.false.
c      if(check_remove) then
c     
c        call backwards(new_group,hoc_points,next_points,
c     >    hoc_backwards,next_backwards)
c     
c        if(debug) then
...
c        end if
c     
c        high_point=hoc_high_points(high_group)
c        do while(high_point .gt. 0)
c          call remove_duplicates(high_group,high_point,
c     >      hoc_high_points,next_high_points,hoc_points,next_points,
c     >      hoc_daughter_points,next_daughter_points,new_group,
c     >      periodic,grid_multiply,d_point,hoc_backwards,next_backwards,
c     >      pos_point_x,pos_point_y,pos_point_z,point_pointer,
c     >      inside,groups_maxx,points_maxx)
c     
