...
c     
      ...
      real cx,cy,cz,mass,pot,d,d2,x,y,z
      ...
      real f_r,force_r,force_t,r,f_x_t,f_y_t,f_z_t
c     
...
c     
          force_r=force_point_x(point)*cx+
     >      force_point_y(point)*cy+
     >      force_point_z(point)*cz
c     
          f_x_t=force_point_x(point)+f_r*cx
          f_y_t=force_point_y(point)+f_r*cy
          f_z_t=force_point_z(point)+f_r*cz
          force_t=sqrt(f_x_t**2+f_y_t**2+f_z_t**2)
c     
          r=sqrt(dx**2+dy**2+dz**2)
c     
          write(99,1)level(group),group,inside(point),
     >      point,point_pointer(point),
     >      r,density(point),-potential_point(point),-pot,
     >      potential_point(point)/pot,
     >      -force_r,f_r,-force_r/f_r,force_t,force_t/f_r
...
c     
        force_r=force_x(part)*cx+
     >    force_y(part)*cy+
     >    force_z(part)*cz
c     
        f_x_t=force_x(part)+f_r*cx
        f_y_t=force_y(part)+f_r*cy
        f_z_t=force_z(part)+f_r*cz
        force_t=sqrt(f_x_t**2+f_y_t**2+f_z_t**2)
c     
        r=sqrt(dx**2+dy**2+dz**2)
c     
        write(98,2)part,group,level(group),r,-potential(part),-pot,
     >    potential(part)/pot,-force_r,f_r,-force_r/f_r,force_t/f_r
      end do
