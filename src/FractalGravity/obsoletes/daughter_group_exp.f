...
c     
      ...
      integer next_daughter_points(maxx),points_in_group(groups_maxx)
      ...
      integer n,i,j,k,adj(-1:1,-1:1,-1:1)
c     
...
c     
      call cpu_time(t_2)
c     
      call test_pos(high_group,hoc_high_points,next_high_points,
...
c
        do k=-1,1
          do j=-1,1
            do i=-1,1
              if((empty(i,j,k) .and. adj(i,j,k) .gt. 0) .or.
     >          (.not. empty(i,j,k) .and. adj(i,j,k) .le. 0)) then
                write(49,*)'mismatch ',i,j,k,empty(i,j,k),adj(i,j,k)
              end if
            end do
          end do
        end do
c        print*,'calling uppers '
        call find_uppers(high_point,hoc_daughter_points,
     >    point_up_x,point_up_y,point_up_z,
     >    next_daughter_points,point_pointer,adj,points_maxx)
c     
        call find_downers(high_point,hoc_daughter_points,
     >    point_down_x,point_down_y,point_down_z,
     >    next_daughter_points,point_pointer,adj,points_maxx)
c     
        high_point=next_high_points(high_point)
