      subroutine find_smart_uppers(high_group,
...
c     
      ...
      integer p_h(27),n_h,n_high,p_p,wrapping,n,m
      integer pointer_h(27),pointer_x,pointer_y,pointer_z
      integer hoc_possibles(maxx),next_possibles(maxx),hoc(4)
...
c
c      write(87,*)'top ',grid_multiply,d_point_2,wrapping
c     
      n=p_1_h/d_point_2+1
      ...
      if(periodic) then
        ...
        if(n .eq. wrapping) then
          ...
          hoc(4)=hoc_possibles(2)
        else if(n .eq. wrapping-1) then
          hoc(3)=hoc_possibles(wrapping)
          hoc(4)=hoc_possibles(1)
        else
          hoc(3)=hoc_possibles(n+1)
          hoc(4)=hoc_possibles(n+2)
        end if
      else
        if(n .eq. 1) then
          hoc(1)=-1
...
c
c      do m=1,4
c        if(hoc(m) .gt. 0) then
c          write(87,*)high_point,n,m,p_1_h,pos_point_x(hoc(m))
c        else
c          write(87,*)high_point,n,m,p_1_h,-1
c        end if
c      end do
c*******
      if(periodic) then
     ...
     >              1+pointer_x+pointer_y*3+pointer_z*9
        ...
        do n=1,4
          h_point=hoc(n)
          do while(h_point .gt. 0)
c            write(87,*)high_point,n,h_point,p_1_h,pos_point_x(h_point)
     ...
     >            then
                  ...
                  pointer_h(n_high)=
     >              1+pointer_x+pointer_y*3+pointer_z*9
c                  write(87,*)high_point,n_high,n,pointer_h(n_high)
                end if
              end if
            end if
            h_point=next_possibles(h_point)
