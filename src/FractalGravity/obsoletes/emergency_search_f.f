      subroutine emergency_search_f(periodic,grid_multiply,
...
c     
      ...
      integer d
c     
      logical periodic,dist_p,dist_p_h
c     
      dist_p(i,j,k)=min(abs(i-j),abs(i-j+k),abs(i-j-k)) .eq. 0
      dist_p_h(i,j,k,d)=min(abs(i-j),abs(i-j+k),abs(i-j-k)) .le. d
c     
...
c     
      if(periodic) then
        ...
        do while(high_point .gt. 0 .and. point_u .lt. 0)
          if(dist_p_h(p_x-delta,pos_point_x(high_point),
     >      grid_multiply,delta)) then
            if(dist_p_h(p_y-delta,pos_point_y(high_point),
     >        grid_multiply,delta)) then
              if(dist_p_h(p_z-delta,pos_point_z(high_point),
     >          grid_multiply,delta)) then
     ...
     >                  then
        ...
        high_point=hoc_high
        do while(high_point .gt. 0 .and. point_u .lt. 0)
          if(abs(p_x-delta-pos_point_x(high_point)) .le. delta) then
            if(abs(p_y-delta-pos_point_y(high_point)) .le. delta) then
              if(abs(p_z-delta-pos_point_z(high_point)) .le. delta) then
                point=hoc_daughter_points(high_point)
                do while(point .gt. 0 .and. point_u .lt. 0)
