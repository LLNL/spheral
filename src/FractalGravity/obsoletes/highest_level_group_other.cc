...
{
  void highest_level_group_other(list <Chain*>& list_chains,Fractal& fractal,Misc& misc)
  {
		...
		for(list <Point*>::const_iterator point_itr=group.list_points.begin();point_itr!=group.list_points.end();++point_itr)
		  {
		    ...
		    for(list <Particle*>::const_iterator p_itr=point.list_other_particles.begin();p_itr!=point.list_other_particles.end();++p_itr)
		      {
