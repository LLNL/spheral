      subroutine better_order(pos_a,pos_b,points_maxx,
...
c
      integer points_maxx
      integer pos_a(points_maxx),pos_b(points_maxx)
...
c
      do n=1,total_b
        if(hoc_b(n) .gt. 0) call better_order_1(pos_a,points_maxx,
     >    hoc_b(n),next_b,grid_multiply,delta,periodic)
      end do
c
      hoc_hoc_up=-1
      do n=1,total_b
