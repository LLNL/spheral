...
c     
      ...
      integer hoc_possibles(maxx),next_possibles(maxx)
      integer n,i,j,l,m
c     
...
c
      write(58,*)'daughter group ',new_group
      memory_value=0
      go_ahead=.false.
...
c     
        ...
        point_0=point
c     
        call find_smart_empties(high_group,high_point,
...
c     >    empty_tmp,groups_maxx,points_maxx)
c
c        write(82,*)'high point ',high_point
c        do n=-1,1
c          do m=-1,1
c            do l=-1,1
c              if(empty(l,m,n) .neqv. empty_tmp(l,m,n))
c     >          write(82,82)l,m,n,empty(l,m,n),empty_tmp(l,m,n)
c 82           format(3i3,2l2)
c            end do
c          end do
c        end do
...
c     
      high_point=hoc_high_points(high_group)
      do while(high_point .gt. 0)
        call find_smart_uppers(high_group,
     >    high_point,hoc_high_points,hoc_possibles,next_possibles,
...
c     
c        call find_uppers(high_group,high_point,hoc_high_points,
c     >    next_high_points,hoc_daughter_points,next_daughter_points,
c     >    periodic,grid_multiply,d_point,
c     >    point_up_x,point_up_y,point_up_z,
c     >    pos_point_x,pos_point_y,pos_point_z,point_pointer,
c     >    groups_maxx,points_maxx)
c     
        high_point=next_high_points(high_point)
      end do
c     
...
c
      call cpu_time(t_7)
      if(t_7-t_1 .gt. 0.1)
     >  write(39,39)new_group,level(new_group),total_points,
     >  t_7-t_1,t_2-t_1,t_3-t_2,
     >  t_4-t_3,t_5-t_4,t_6-t_5,t_7-t_6
 39   format(3i7,7(1pe12.4))
      return
      end
