...
{
  void power_spectrum(double* rhoR,const int& length,vector <double>& variance_rho,vector <double>& variance_pot,
		      vector <double>& variance_force,vector <double>& variance_force_s,const int& lev,const double& d0,const bool& do_var)
  {
    ...
    double grav_const=4.0*pi/(double)(length*length);
	...
	for(int ky=0;ky < length;++ky)
	  {
		...
		double square=pow(rhoR[holy_grail],2)+pow(rhoR[holy_grail+1],2);
		...
		if(square > 0.0)
		  {
		    ...
		    double pot=square*pow(grav_const/(green_2[ka]+green_2[kb]+green_2[kc]+1.0e-30),2);
