...
namespace FractalSpace
{
  void particle_lists_fixed(vector <vector <Group*> >& all_groups,Fractal& fractal,Misc& misc)
  {
    ...
    double z_max=1.0;
    if(!period)z_max=1.0/(double)fractal.get_length_ratio();
    double x,y,z;
	...
	if(doit)
	  {
	    const int n_x=(int)floor(x*grid_length);
	    const int n_y=(int)floor(y*grid_length);
	    const int n_z=(int)floor(z*grid_length);
	    const int pp=n_x+(n_y+n_z*length)*length;
	    //
	  ...
	  {
	    cout << "particle outside " << x << " " << y << " " << z << " " << particle << endl;
	  }
      }
		...
		if(point.get_real_pointer() == 0)
		  {
			...
			int p_x=((int)floor(x*grid_multiplier)-point.get_pos_point_x())/d_inv;
			int p_y=((int)floor(y*grid_multiplier)-point.get_pos_point_y())/d_inv;
			int p_z=((int)floor(z*grid_multiplier)-point.get_pos_point_z())/d_inv;
			assert(p_x == 0 || p_x == 1);
			assert(p_y == 0 || p_y == 1);
			assert(p_z == 0 || p_z == 1);
			Point* p_p=&point;
