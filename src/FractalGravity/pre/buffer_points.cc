...
namespace FractalSpace
{
  void buffer_points(Group& group, Fractal& fractal,Misc& misc)
  {
    ...
    double scale=(double)(fractal.get_grid_length()*Misc::pow(2,fractal.get_level_max()));
	...
	for(vector<Point*>::const_iterator point_itr=group.p_list_really_high.begin();point_itr != group.p_list_really_high.end();++point_itr)
	  {
		...
		n_x=((int)(pos[0]*scale)-p_x)/width;
		n_y=((int)(pos[1]*scale)-p_y)/width;
		n_z=((int)(pos[2]*scale)-p_z)/width;
		//
		assert(n_x==0 || n_x==1);
		assert(n_y==0 || n_y==1);
		assert(n_z==0 || n_z==1);
	...
	for(vector <Point*>::const_iterator point_itr=group.list_points.begin();point_itr != group.list_points.end();++point_itr)
	  {
	    //--------------------------------------------------------------------------------------------------------------------------------
	    // for top group in isolated BC, make sure edge has no high points
	    //--------------------------------------------------------------------------------------------------------------------------------
	    Point& point=**point_itr;
	    point.set_inside_high();
	  }
      }
    int n_h=0;
    for(vector <Point*>::const_iterator point_itr=group.list_points.begin();point_itr != group.list_points.end();++point_itr)
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	if((*point_itr)->get_it_is_high()) n_h++;
      }
