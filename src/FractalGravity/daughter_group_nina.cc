...
namespace FractalSpace
{
  ...
  { 
    int length_3=fractal.get_grid_length();
    ...
    //    cout << "tot dau " << mem.total_points_used << " " << mem.total_points_generated;
    //    cout << " " << new_points_gen << endl;
    int new_counter=0;
	...
	for(int p=0;p<27;++p)
	  {
	    ...
	    // lower position value will be generated by neighboring high_points. Algorithm is conservative,
	    // it will sometimes generate more than one point at a position, but never zero. Duplicates are removed later
	    //--------------------------------------------------------------------------------------------------------------------------------
    ...
    //    bool badd=test_group(new_group);
    fractal.timing(1,14);
    fractal.timing(-1,16);
