...
namespace FractalSpace
{
  void initial_forces_sharp(Fractal_Memory& fractal_memory,Fractal& fractal,Misc& misc)
  {
		...
		for(int kz=0;kz <= nyq ; kz++)
		  {
		    ...
		    // crock crock crock
		    //		    if(!(lev == 0 && kc == 1  && ky ==0 &&  kx == 0))
		    //		       amplitude_raw*=1.0e-5;
		    //
		    double angle=0.0;
		    if(nyquist)
		      {
	...
	power_spectrum(pot,length,variance_rho,variance_pot,variance_force,variance_force_s,lev,fractal.get_density_0(),true);
	//
