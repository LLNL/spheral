...
splatMultipleFieldsMash(const FieldListSet<Dimension>& fieldListSet,
  ...
  for (InternalNodeIterator<Dimension> nodeItr = position.internalNodeBegin();
    ...
    if (flagNodeDone(nodeItr) == 0) {
      ...
      position.setMasterNeighborNodeLists(position(nodeItr), Hfield(nodeItr), masterLists, coarseNeighbors);
      samplePositions.setMasterNeighborNodeLists(position(nodeItr), Hfield(nodeItr), masterListsSample, coarseNeighborsSample);
      ...
      for (MasterNodeIterator<Dimension> masterItr = position.masterNodeBegin(masterLists);
        ...
        samplePositions.setRefineNeighborNodeLists(ri, Hi, coarseNeighborsSample, refineNeighborsSample);
