...
initialize(const typename Dimension::Scalar time,
  ...
  const TableKernel<Dimension>& WT = this->kernel();
...
//             const auto fij = epsTensile*pow(Wi/(Hdeti*WnPerh), nTensile);
      ...
      // HLLC approximate Reimann Solver
      

      //const auto Prhoi = Pstar/(rhoi*rhoj);
      const auto Prho = Pstar/(rhoi*rhoj);
      //const auto Prho = Pstar/(rhoi*rhoj)+QPiji.Trace()/3.0;
      const auto deltaDvDt = Prho*(gradWi + gradWj);
      DvDti -= mj*deltaDvDt;
      DvDtj += mi*deltaDvDt;
      if (compatibleEnergyEvolution) pairAccelerations[kk] = -mj*deltaDvDt;  // Acceleration for i (j anti-symmetric)

      auto deltaDvDxi = (vi-vstar).dyad(gradWi);
      auto deltaDvDxj = (vstar-vj).dyad(gradWj);
