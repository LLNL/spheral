...
evaluateDerivatives(const typename Dimension::Scalar time,
  ...
  auto  newInterfaceFraction = derivatives.fields(ReplaceBoundedState<Dimension, SymTensor>::prefix() + FSIFieldNames::interfaceFraction, 0.0);
  auto  newInterfaceSmoothness = derivatives.fields(ReplaceBoundedState<Dimension, SymTensor>::prefix() + FSIFieldNames::interfaceSmoothness, 0.0);
  auto  newInterfaceNormals = derivatives.fields(ReplaceBoundedState<Dimension, SymTensor>::prefix() + FSIFieldNames::interfaceNormals, Vector::zero);
