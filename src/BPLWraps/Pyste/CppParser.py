...
class CppParser:
    ...
    def SelectGCCVersion(self):
        try:
            return os.environ["GCCXML_COMPILER"]
        except:
            f = os.popen("g++ -dumpversion")
            ver = f.readline()[:-1]
            f.close()
            gccstring = "g++-%s" % ver
            if gccstring.count(".") == 2:
                assert gccstring[-2] == "."
                gccstring = gccstring[:-2]
            return gccstring
    ...
    def ParseWithGCCXML(self, header, tail):
        ...
        try:            
            ...
            # call gccxml
            cmd = '%s %s %s "%s" -fxml=%s --gccxml-compiler %s' 
            filename = self.Unixfy(filename)
            xmlfile = self.Unixfy(xmlfile)
            gccstring = self.SelectGCCVersion()
            status = os.system(cmd % (self.gccxml_path, includes, defines, filename, xmlfile, gccstring))
