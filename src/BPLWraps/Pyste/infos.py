...
# ExportedInfo
#==============================================================================
class ExportedInfo(DeclarationInfo):
    """
    Explicitly declare that the given C++ type is exported to python
    *somewhere*.
    """

    def __init__(self, cppName):
        DeclarationInfo.__init__(self)
        self._Attribute('name', cppName)
        exporters.external_exported_names.append(cppName)
# JMO end
...
# JMO begin
def iterator(info, name, beginIterator, endIterator):
    iter = info._Attribute('__iterators__')
    if iter is None:
        info._Attribute('__iterators__', [(name, beginIterator, endIterator)])
    else:
        iter.append((name, beginIterator, endIterator))

def add_property(info, name, getMethod, setMethod=None):
    add_property = info._Attribute('__add_property__')
    if add_property is None:
        info._Attribute('__add_property__', [(name, getMethod, setMethod)])
    else:
        add_property.append((name, getMethod, setMethod))
# JMO end
