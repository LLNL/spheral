...
# JMO begin
    def GetIteratorMethods(self):
        iterator_methods = self.info.__iterators__
        result = []
        if iterator_methods:
            for name, beginName, endName in iterator_methods:
                beginDecls = [x for x in self.public_members if x.name == beginName]
                endDecls = [x for x in self.public_members if x.name == endName]
                if len(beginDecls) > 0 and len(endDecls) > 0:
                    result.append((name, beginDecls[0], endDecls[0]))
        return result

    def GetAddPropertyMethods(self):
        property_methods = self.info.__add_property__
        result = []
        if property_methods:
            for name, getMethod, setMethod in property_methods:
                getDecls = [x for x in self.public_members if
                            x.name == getMethod and len(x.parameters) == 0]
                setDecls = [None]
                if setMethod is not None:
                    setDecls = [x for x in self.public_members if
                                x.name == setMethod and len(x.parameters) == 1]
                if len(getDecls) > 0 and len(setDecls) > 0:
                    result.append((name, getDecls[0], setDecls[0]))
                elif len(getDecls) > 0:
                    result.append((name, getDecls[0], None))
        return result
# JMO end
...
# JMO begin
        # export any iterator methods
        iteratorMethods = self.GetIteratorMethods()
        for name, beginDecl, endDecl in iteratorMethods:
            beginPointer = beginDecl.PointerDeclaration()
            endPointer = endDecl.PointerDeclaration()
            policy = self.info[beginDecl.name].policy or ''
            if policy:
                policyCode = '<%s%s, %s >' % (namespaces.python, policy.Code(),
                                              self.class_.FullName())
            else:
                policyCode = ''
            code = '.def("%s", range%s(%s, %s))' % (name,
                                                    policyCode,
                                                    beginPointer,
                                                    endPointer)
            self.Add('inside', code)

        # export any add_property attributes
        addPropertyMethods = self.GetAddPropertyMethods()
        for name, getMethod, setMethod in addPropertyMethods:

            code = '.add_property'

            # Check for policy info on the get method.
            getMethod_info = self.info[getMethod.name]
            getMethod_info.policy = exporterutils.HandlePolicy(getMethod,
                                                               getMethod_info.policy)
            policy = getMethod_info.policy or None
            if policy:
                policy = '%s%s()' % (namespaces.python, policy.Code())
                code += '<' + policy + ' >'

            getPointer = getMethod.PointerDeclaration()
            if setMethod is None:
                code += '("%s", %s)' % (name, getPointer)
            else:
                setPointer = setMethod.PointerDeclaration()
                code += '("%s", %s, %s)' % (name,
                                            getPointer,
                                            setPointer)
            self.Add('inside', code)
...
# JMO begin
    param_refs = [isinstance(x, ReferenceType) for x in m.parameters[:count]]
    param_ptrs = [isinstance(x, PointerType) for x in m.parameters[:count]]
# JMO end
    ...
    params = ', '.join(params) 
# JMO begin
    return params, param_names, param_types, param_refs, param_ptrs
# JMO end
...
class _VirtualWrapperGenerator(object):
    ...
    def Declaration(self, method, indent):
        ...
        if result == 'void':
            return_str = ''
# JMO begin
        params, param_names, param_types, param_refs, param_ptrs = _ParamsInfo(method)
# JMO end
        constantness = ''
        if method.const:
            constantness = ' const'
...
# JMO changes: account for reference and pointer arguments.
        bad_types_bases = ['double', 'int', 'bool']
        bad_types = bad_types_bases + \
                    ['const ' + x for x in bad_types_bases] + \
                    [x + ' const' for x in bad_types_bases]
        bad_ref_types = [x + '&' for x in bad_types]
        bad_ptr_types = [x + '*' for x in bad_types]
        param_names_str = ''
        for i in xrange(len(param_names)):
            if param_refs[i] and param_types[i] not in bad_ref_types:
                param_names_str += 'boost::ref(%s)' % param_names[i]
            elif param_ptrs[i] and param_types[i] not in bad_ptr_types:
                param_names_str += 'boost::ptr(%s)' % param_names[i]
            else:
                param_names_str += param_names[i]
...
# JMO begin
                params, param_names, param_types, param_refs, param_ptrs = _ParamsInfo(method, argNum)
# JMO end
                decl += '\n'
...
# JMO begin
            base_methods = [copy.deepcopy(x) for x in base
                            if (IsVirtual(x) and not self.info[x.name].no_override)]
            class_method_sigs = [MethodSig(x) for x in self.class_ if IsVirtual(x)]
            for base_method in base_methods:
                if MethodSig(base_method) not in class_method_sigs:
...
# JMO begin
                params, param_names, param_types, param_refs, param_ptrs = _ParamsInfo(cons, argNum)
# JMO end
                if params:
                    params = ', ' + params
