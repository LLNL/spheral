...
# JMO begin
    context['Exported'] = infos.ExportedInfo
# JMO end
    # functions
...
# JMO begin
    context['iterator'] = infos.iterator
    context['add_property'] = infos.add_property
# JMO end
    # policies
...
# JMO begin
                try:
                    interfaces_order = ReadInterfaceOrder()
                    return GenerateMain(module, out, interfaces_order) #OrderInterfaces(parser, exporters.exporters))
                except:
                    print "Unable to load interface order: regenerating..."
                    return GenerateMain(module, out, OrderInterfaces(parser, exporters.exporters))
# JMO end
        else:
...
# JMO begin
def OldOrderInterfaces(interfaces):
# JMO end
...
# JMO begin
import ClassExporter
def OrderInterfaces(parser, exports, exported_names={}):
    start = time.clock()

    # Do some magic so that the export.class_ member is available.
    ...
    print '%0.2f seconds spent doing "ExpandTypedefs"' % (time.clock()-start)

    # Build a map of exported classes and their interfaces.
    mapClassToInterface = {}
    for export in exports:
        if isinstance(export, ClassExporter.ClassExporter):
            mapClassToInterface[export.class_.FullName()] = export.interface_file

    # Prepare the interface dependencies map.
    interfaceDependencies = {}
    for export in exports:
        interfaceDependencies[export.interface_file] = []

    # Now check to see if there are any classes exported in one interface
    # that are needed as bases classes in another.
    for export in exports:
        if isinstance(export, ClassExporter.ClassExporter):
            for base in export.ClassBases():
                baseName = base.FullName()
                if baseName in mapClassToInterface.keys():
                    if export.interface_file != mapClassToInterface[baseName]:
                        if export.interface_file in interfaceDependencies.keys():
                            if mapClassToInterface[baseName] not in \
                                   interfaceDependencies[export.interface_file]:
                                interfaceDependencies[export.interface_file].append(
                                    mapClassToInterface[baseName])
                        else:
                            interfaceDependencies[export.interface_file] = \
                                                 [mapClassToInterface[baseName]]

    # interfaceDependencies currently only reflects the first level (or direct)
    # dependencies.  Now build the full dependencies all the way down the chain
    # for each interface.
    fullDependencies = {}
    for interface in interfaceDependencies:
        fullDependencies[interface] = interfaceDependencies[interface]
        done = False
        while not done:
            done = True
            for dep1 in fullDependencies[interface]:
                for dep2 in interfaceDependencies[dep1]:
                    if dep2 not in fullDependencies[interface]:
                        fullDependencies[interface].append(dep2)
                        done = False

    # Now build the final dependency based ordering.  I know, this is a bubble
    # sort algorithm but compared to the time spent parsing the headers above this
    # should a minor expense.
    interfaceOrder = fullDependencies.keys()[:]
    done = False
    while not done:
        done = True
        for i in xrange(len(interfaceOrder)):
            for j in xrange(i + 1, len(interfaceOrder)):
                if interfaceOrder[j] in fullDependencies[interfaceOrder[i]]:
                    assert interfaceOrder[i] not in fullDependencies[interfaceOrder[j]]
                    interfaceOrder[i], interfaceOrder[j] = interfaceOrder[j], interfaceOrder[i]
                    done = False

    # That's it, return the ordering.
    print '%0.2f seconds spent ordering interfaces' % (time.clock()-start)
    return interfaceOrder



import pickle
def CacheInterfaceOrder(interfaceOrder,
                        cacheFile = ".InterfaceOrder"):
    f = open(cacheFile, "w")
    p = pickle.Pickler(f)
    p.dump(interfaceOrder)
    f.close()
    return 0


def ReadInterfaceOrder(cacheFile = ".InterfaceOrder"):
    f = open(cacheFile)
...
# JMO begin
    # Add the C++ names the user specified are externally exported.
    for external_name in exporters.external_exported_names:
        exported_names[external_name] = None
# JMO end

    # order the exports
# JMO begin
    interfaces_order = OrderInterfaces(parser, exports, exported_names)
    CacheInterfaceOrder(interfaces_order)
# JMO end
    order = {}
    for export in exports:
