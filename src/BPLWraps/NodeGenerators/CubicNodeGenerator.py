...
class GenerateSquareNodeDistribution(NodeGeneratorBase):
    ...
    def __init__(self,
        ...
        lx = xmax[0] - xmin[0]
        ly = xmax[1] - xmin[1]
        nxdomains = int(sqrt(lx/ly * procs) + 0.1)
        nydomains = int(procs/nxdomains + 0.1)
        assert nxdomains * nydomains == procs
        ...
        ixdomain = rank % nxdomains
        iydomain = rank / nxdomains
    ...
    def makeHround(self):
        for i in xrange(len(self.H)):
            h0 = sqrt(self.H[i].Determinant())
            self.H[i] = SymTensor2d.one * h0
        return
...
class GenerateCubicNodeDistribution(NodeGeneratorBase):
    ...
    def __init__(self,
        ...
        ly = xmax[1] - xmin[1]
        lz = xmax[2] - xmin[2]
        nxdomains = int((lx*lx/(ly*lz) * procs)**(1.0/3.0) + 0.1)
        nydomains = int(ly/lx * nxdomains + 0.1)
        nzdomains = int(lz/lx * nxdomains + 0.1)
        assert nxdomains * nydomains * nzdomains == procs
        ...
        # Compute our domain indicies.
        ixdomain = rank % nxdomains
        iydomain = (rank / nxdomains) % nydomains
        izdomain = rank / (nxdomains*nydomains)
        ixmin = nodeindex(ixdomain, nxperdomain,  nxremainder)
        ixmax = nodeindex(ixdomain + 1, nxperdomain,  nxremainder)
        iymin = nodeindex(iydomain, nyperdomain,  nyremainder)
        iymax = nodeindex(iydomain + 1, nyperdomain,  nyremainder)
