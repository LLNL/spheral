...
def findLastRestart(baseName):
    ...
    cycles = findAvailableRestartCycles(baseName)
...
def findAvailableRestartCycles(baseName):
    ...
    try:
        import mpi
        baseName += '_rank%i_of_%idomains' % (mpi.rank, mpi.procs)
    except:
        pass
    ...
    cycles = [int(x) for x in
              [(y.split("cycle")[-1]).split(".gz")[0] for y in restartFiles]]
...
def loadLastConservationHistory(baseName):
    from Spheral import DataBase1d, AccessType, FlatFileIO
    ...
    f = FlatFileIO(restartFile, AccessType.Read)
