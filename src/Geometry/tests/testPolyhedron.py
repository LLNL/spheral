...
#        self.polyhedron = Polyhedron(self.points)
    ...
    # #---------------------------------------------------------------------------
    # def testPlanar(self):
    #     verts = vector_of_Vector3d()
    #     verts.append(Vector3d(0,0,0))
    #     verts.append(Vector3d(0,1,0))
    #     verts.append(Vector3d(1,0,0))
    #     verts.append(Vector3d(1,1,0))
    #     verts.append(Vector3d(0,0,1))
    #     verts.append(Vector3d(0,1,1))
    #     verts.append(Vector3d(1,0,1))
    #     verts.append(Vector3d(1,1,1))
    #     box = Polyhedron(verts)
    #     for facet in box.facets():
    #         self.failUnless(facet.planar(),
    #                         "Failed test that facet with normal %s is planar for box." % facet.normal)

    #---------------------------------------------------------------------------
    # Test !planar
    #---------------------------------------------------------------------------
    def testNotPlanar(self):
        verts = vector_of_Vector3d()
        verts.append(Vector3d(0,0,0))  # 0
        verts.append(Vector3d(0,1,0))  # 1
        verts.append(Vector3d(1,0,0))  # 2
        verts.append(Vector3d(1,1,0))  # 3
        verts.append(Vector3d(0,0,1))  # 4
        verts.append(Vector3d(0,1,1))  # 5
        verts.append(Vector3d(1,0,1))  # 6
        verts.append(Vector3d(1,1,1))  # 7
        facetVerts = vector_of_vector_of_unsigned(6, vector_of_unsigned(4))
        for i, vals in [(0, (0, 1, 3, 2)),
                        (1, (1, 5, 7, 3)),
                        (2, (4, 6, 7, 5)),  # <- non-planar facet
                        (3, (0, 2, 6, 4)),
                        (4, (2, 3, 7, 6)),
                        (5, (0, 4, 5, 1))]:
            for j, v in enumerate(vals):
                facetVerts[i][j] = v
        print [list(x) for x in facetVerts]
        box = Polyhedron(verts, facetVerts)
        facets = box.facets()
        assert len(facets) == 6
        for i in xrange(6):
            if i == 2:
                self.failUnless(not facets[i].planar(),
                                "Failed test that facet with normal %s is NOT planar for box." % facet.normal)
            else:
                self.failUnless(facets[i].planar(),
                                "Failed test that facet with normal %s is planar for box." % facet.normal)
