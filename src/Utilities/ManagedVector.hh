...
template <typename DataType> class ManagedVector;

template <typename U> ManagedVector<U> deepCopy(ManagedVector<U> const &array);

template <typename U>
bool compare(ManagedVector<U> const &, ManagedVector<U> const &);

template <typename DataType>
class ManagedVector : private chai::ManagedArray<DataType> {
...
public:
  ...
  // Copy Constructor
  // ---------------------
  SPHERAL_HOST_DEVICE constexpr inline ManagedVector(
      ManagedVector const &rhs) noexcept
      : MA(rhs), m_size(rhs.m_size) {}

  // ---------------------
  // Assignment
  // ---------------------
  SPHERAL_HOST_DEVICE ManagedVector<DataType> &
  operator=(ManagedVector const &rhs) {
    MA::operator=(rhs);
    m_size = rhs.m_size;
    return *this;
  }
  ...
  SPHERAL_HOST_DEVICE ManagedVector<DataType> &operator=(std::nullptr_t) {
    MA::operator=(nullptr);
    return *this;
  }
  ...
  // SPHERAL_HOST operator std::vector<DataType>() const { return
  // std::vector<DataType>(begin(), end()); }

  template <typename U = ManagedVector<DataType>>
  SPHERAL_HOST void setCallback() {
    MA::setUserCallback(getCallback());
  }

  template <typename U = ManagedVector<DataType>>
  SPHERAL_HOST auto getCallback() {
    // #ifdef SPHERAL_CALLBACK_ENABLED
    //     std::string const typeString = LvArray::system::demangleType< U >();
    //     return [typeString] (const chai::PointerRecord* record, chai::Action
    //     action, chai::ExecutionSpace exec) {
    //         std::string const size =
    //         LvArray::system::calculateSize(record->m_size); std::string const
    //         paddedSize = std::string( 9 - size.size(), ' ' ) + size; char
    //         const * const spaceStr = ( exec == chai::CPU ) ? "HOST  " :
    //         "DEVICE";
    //
    //         if (action == chai::Action::ACTION_MOVE){
    //           SPHERAL_LOG(Info, "Moved " << paddedSize << " to the " <<
    //           spaceStr << ": " << typeString << " @ " <<
    //           record->m_pointers[exec] )
    //         }
    //         if (action == chai::Action::ACTION_ALLOC){
    //           SPHERAL_LOG(Info, "Allocated on " << spaceStr << " " <<
    //           paddedSize << " : " << typeString << " @ " <<
    //           record->m_pointers[exec] )
    //         }
    //         if (action == chai::Action::ACTION_FREE){
    //           SPHERAL_LOG(Info, "Deallocated " << paddedSize << " : " <<
    //           typeString << " @ " <<  record->m_pointers[exec] )
    //         }
    //       };
    // #else
    return
        [](const chai::PointerRecord *, chai::Action, chai::ExecutionSpace) {};
    // #endif
  }
...
private:
  ...
  ManagedVector(MA const &managed_array)
      : MA(managed_array), m_size(managed_array.size()) {}
...
template <typename U>
inline bool compare(ManagedVector<U> const &lhs, ManagedVector<U> const &rhs) {
...
template <typename U>
inline ManagedVector<U> deepCopy(ManagedVector<U> const &array) {
