...
def build_deps(args):
  ...
  for s in spec_list:
    ...
    if sexe("{0} dev-build --deprecated --quiet -d {1} -u initconfig spheral@develop%{2} 2>&1 | tee -a \"dev-build-{2}-out.txt\"".format(spack_cmd, os.getcwd(), s), echo=True) : sys.exit(1)
