...
def build_deps(args):
  ...
  if not args.init_only:
      for s in spec_list:
        ...
        if sexe("{0} install --fresh --only dependencies {2}@develop%{3} 2>&1 | tee -a \"dev-build-{3}-out.txt\"".format(spack_cmd, os.getcwd(), package_name, s), echo=True) : sys.exit(1)
        # Using dev-build we can have spack generate an init-config with the local source files for spheral.
        if sexe("{0} dev-build --fresh -u initconfig -i {2}@develop%{3} 2>&1 | tee -a \"dev-build-{3}-out.txt\"".format(spack_cmd, os.getcwd(), package_name, s), echo=True) : sys.exit(1)
