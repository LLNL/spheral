...
#ATS:    t10 = test(SELF, "--graphics False --clearDirectories True --domainIndependent True --outputFile 'TensileRod-GradyKipp-1d-1proc-reproducing.txt' --referenceFile 'Reference/TensileRod-GradyKipp-1d-1proc-reproducing-darwin-20130907.txt'", np=1, label="Tensile rod domain independence test SERIAL RUN")
#ATS:    t11 = testif(t10, SELF, "--graphics False --clearDirectories False --domainIndependent True --outputFile 'TensileRod-GradyKipp-1d-4proc-reproducing.txt' --comparisonFile 'TensileRod-GradyKipp-1d-1proc-reproducing.txt' --referenceFile 'Reference/TensileRod-GradyKipp-1d-1proc-reproducing-darwin-20130907.txt'", np=4, label="Tensile rod domain independence test 4 DOMAIN RUN")
#ATS:else:
...
commandLine(length = 3.0,
            ...
            referenceFile = "Reference/TensileRod-GradyKipp-1d-1proc-reproducing-20130918.txt",
