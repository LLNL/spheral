...
nx1, ny1 = 40, 20 #40, 400
nx2, ny2 = 40, 20 #10, 100
nPerh = 2.01

rho1, rho2 = 1.0, 1.0 #1.0, 0.25
P1, P2 = 1.0, 1.0 #1.0, 0.1795
...
restartStep = 20
restartBaseName = "PlanarShear-2d-%ix%i" % (nx1, ny1 + ny2)
...
if asph:
    nodes1 = AsphNodeList2d(eos, 0)
    nodes2 = AsphNodeList2d(eos, 0)
else:
    nodes1 = SphNodeList2d(eos, 0)
    nodes2 = SphNodeList2d(eos, 0)
output('nodes1')
output('nodes2')
...
WT = TableKernel2d(NBSplineKernel2d(5), 100)
WTPi = WT
output('WT')
output('WTPi')
...
if restoreCycle is None:
    ...
    for nodeID in xrange(nodes1.numNodes):
        nodes1.velocity()[nodeID] = Vector2d(vx1, vy1)
    for nodeID in xrange(nodes2.numNodes):
        nodes2.velocity()[nodeID] = Vector2d(vx2, vy2)
...
##for nodeID in xrange(nodes1.numNodes):
##    nodes1.specificThermalEnergy[nodeID] = P1/((gamma - 1.0)*nodes1.massDensity[nodeID])
##for nodeID in xrange(nodes2.numNodes):
##    nodes2.specificThermalEnergy[nodeID] = P2/((gamma - 1.0)*nodes2.massDensity[nodeID])
...
### Plot the limiter estimate of grad div v against the analytic solution.
##r = array([0.0]*nodes1.numInternalNodes)
##divV = array([0.0]*nodes1.numInternalNodes)
##gradDivV = array([0.0]*nodes1.numInternalNodes)
##sig = q.sigma
##gdv = q.gradDivVelocity
##for nodeID in xrange(nodes1.numInternalNodes):
##    xi = nodes1.positions[nodeID]
##    r[nodeID] =  xi.y
##    divV[nodeID] = sig[0][nodeID].Trace()
##    gradDivV[nodeID] = gdv[0][nodeID].magnitude()

##DivPlot = Gnuplot.Gnuplot()
##DivData = Gnuplot.Data(r, divV, title='simulation div v')
##gdvData = Gnuplot.Data(r, gradDivV, title='simulation grad div v')
##DivPlot.replot(DivData)
##DivPlot.replot(gdvData)

### A function to compute and plot the div V and del cross V terms against y
##import Numeric
##def plotLimiter(sigmaFieldList):
##    n = 0
##    for field in sigmaFieldList:
##        n += field.nodeList.numInternalNodes

##    yArray = Numeric.array([0.0]*n)
##    divVArray = Numeric.array([0.0]*n)
##    delCrossVArray = Numeric.array([0.0]*n)
##    i = -1
##    for field in sigmaFieldList:
##        for j in xrange(field.nodeList.numInternalNodes):
##            i += 1
##            yArray[i] = field.nodeList.positions[j].y
##            divVArray[i] = abs(field[j].Trace())
##            delCrossVArray[i] = abs(field[j].yx - field[j].xy)
##    den = divVArray + delCrossVArray + 1.0e-20
##    limiterArray = divVArray/(divVArray + delCrossVArray + 1.0e-4)
##    divVdata = Gnuplot.Data(yArray, divVArray,
##                            with = 'points',
##                            title = 'div v')
##    delCrossVdata = Gnuplot.Data(yArray, delCrossVArray,
##                                 with = 'points',
##                                 title = 'del cross v')
##    limiterData = Gnuplot.Data(yArray, limiterArray,
##                               with = 'points',
##                               title = 'Limiter')
##    plot = Gnuplot.Gnuplot()
##    plot.replot(divVdata)
##    plot.replot(delCrossVdata)
##    plot.replot(limiterData)
##    print 'min, max div V', min(divVArray), max(divVArray)
##    print 'min, max del cross V', min(delCrossVArray), max(delCrossVArray)
##    print 'min, max limiter', min(limiterArray), max(limiterArray)
##    return plot

##plot0 = plotLimiter(q.sigma)
##plot0.title('step 0')
##control.step()
##plot1 = plotLimiter(q.sigma)
##plot1.title('step 1')
##control.step()
##plot2 = plotLimiter(q.sigma)
##plot2.title('step 2')

### Plot the various components of the sigma field against y.
##sig = q.sigma
##xxplot = plotFieldList(sig,
##                       xFunction = '%s.y',
##                       yFunction = '%s.xx',
##                       plotStyle = 'points',
##                       winTitle = 'sigma_xx',
##                       userYRange = [-0.5, 0.5])
##xyplot = plotFieldList(sig,
##                       xFunction = '%s.y',
##                       yFunction = '%s.xy',
##                       plotStyle = 'points',
##                       winTitle = 'sigma_xy')
##yxplot = plotFieldList(sig,
##                       xFunction = '%s.y',
##                       yFunction = '%s.yx',
##                       plotStyle = 'points',
##                       winTitle = 'sigma_yx')
##yyplot = plotFieldList(sig,
##                       xFunction = '%s.y',
##                       yFunction = '%s.yy',
##                       plotStyle = 'points',
##                       winTitle = 'sigma_yy')

## Plot the kernel field.
#import PlotKernel2d
#hPlot = PlotKernel2d.plotKernel2d(nodes1, xmin=0.4, ymin=0.4, xmax=0.7, ymax=0.7)

##import SpheralGnuPlotUtilities
##integrator.setGhostNodes()
##integrator.enforceBoundaries()
##periodicplot = SpheralGnuPlotUtilities.plotBoundaryNodes(db, xbc0)
