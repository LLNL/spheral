#-------------------------------------------------------------------------------
# This is a version of the Rayleigh-Taylor problem, as outlined in Doug
# Miller's PhD dissertation, which can be found at:
# https://llnl.primo.exlibrisgroup.com/discovery/delivery/01LLNL_INST:01LLNL_INST/1245691240006316?lang=en
# The RT section begins on page 45.
#
# This version of RT has constant density ideal gases balanced against gravity
# by increasing specific energy with depth.  The density jump is 4x.  The
# acceleration, g, is 1.0e7.  
#
# In the early growth stages, when the growth is still small compared to the
# wavelength of a perturbation, a pure Fourier mode is expected to grow
# according to exp(w*t), each mode with it's own value for 'w'.  The growth
# rate, w, for incompressible fluids depends on the wavenumber k by the
# following relationship:
#
#     w^2 = g k (rho_+ - rho_-)/(rho_+ + rho_-)
#
# So for our parameters, w/sqrt(k) = 774.6, in the case of a perfect
# calculation (this will be regardless of wavenumber k).  In practice, higher
# resolutions calculations will approach this value from below, hitting perhaps
# 750, while low resolutions will score, at best, about half the theoretical
# value, if SPH behaves like gridded codes have in the past.
#-------------------------------------------------------------------------------
import shutil from math import * import mpi
