...
title("3-D hydro test -- Cylindrical Noh problem in RZ coordinates.")
...
restartStep = 10000
dataDir = "Noh-cylindrical-rz-%ix%i" % (nz, nr)
restartDir = dataDir + "/restarts"
visitDir = dataDir + "/visit"
restartBaseName = restartDir + "/Noh-cylindrical-rz"
...
if restoreCycle is None:
    ...
    # Node velocities
    nodes1.setVelocity(VectorField3d("tmp", nodes1, Vector3d(0.0, v1, 0.0)))
...
#-------------------------------------------------------------------------------
xPlane0 = Plane3d(Vector3d(xmin[0], 0.0, 0.0), Vector3d(1.0, 0.0, 0.0))
xPlane1 = Plane3d(Vector3d(xmax[0], 0.0, 0.0), Vector3d(-1.0, 0.0, 0.0))
...
output("hydro.haveBoundary(xbc1)")
...
def filterOnRadius(data, r, rmin, rmax):
    ...
    return [data[i] for i in xrange(len(data)) if (r[i].y >= rmin and r[i].y <= rmax)]
