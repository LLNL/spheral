...
#-------------------------------------------------------------------------------
            # materials
commandLine(nx1 = 20,     # number of nodes
            nx2 = 100,
            rho1 = 2700.0,  # density
            rho2 = 2700.0,
            P1 = 1.0e5,     # Pressure
...
if eos=="stiffenedGas":
    ...
    eos1 = TillotsonEquationOfState(materialName="aluminum",
                    etamin=0.01,
                    etamax=100.0,          
                    units=units)
    eos2 = TillotsonEquationOfState(materialName="aluminum",
                    etamin=0.01,
                    etamax=100.0,
                    units=units)
    rho1 = eos1.referenceDensity
    rho2 = eos2.referenceDensity
elif eos == "aneos":
    import copy
    eos1 = ANEOS(material,
          constants = units)
    ...
    raise ValueError, "wrong eos option"
...
if eos=="stiffenedGas":
    eps1 = (P1+gamma1*Po1)/((gamma1 - 1.0)*rho1)
    eps2 = (P2+gamma2*Po2)/((gamma2 - 1.0)*rho2)
else:
    eps1 = eos1.specificThermalEnergyForPressure(Ptarget = P1,
                                                rho = rho1,
                                                epsMin = 0,
                                                epsMax = 1e8,
                                                epsTol = 1.0,
                                                Ptol = 1.0,
                                                maxIterations = 1000)
    eps2 = eos1.specificThermalEnergyForPressure(Ptarget = P2,
                                                rho = rho2,
                                                epsMin = 0,
                                                epsMax = 1e8,
                                                epsTol = 1.0,
...
for nodes in nodeSet:
    ...
    for i in xrange(nodes.numInternalNodes):
        ...
        vel[i] = Vector(velocity_initial(pos[i].x))
...
if crksph:
    hydro = CRKSPH(dataBase = db,
                   ...
                   XSPH = XSPH,
                   densityUpdate = IntegrateDensity,#densityUpdate,
                   HUpdate = HUpdate)
elif psph:
