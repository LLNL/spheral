...
class DEMInflow:
    ...
    def addParticles(self,particles):
        ...
        cId0 = max(self.db.DEMCompositeParticleIndex.max(),0)
        uId0 = max(self.db.DEMUniqueIndex.max(),0)
        ...
        if mpi.rank == 0:
            ...
            for i in range(numParticles):
                for j in range(particles[i].numSubParticles):
                    ...
                    vel[k] = self.inflowVelocity+0.1*Vector((random.random()-0.5),(random.random()-0.5),(random.random()-0.5))
    ...
    def __call__(self,cycle,time,dt):
        ...
        if (time - self.tGen >= self.dtGen):
            ...
            for i in range(self.nodeList.numInternalNodes):
                maxPosz = max(maxPosz,pos[i].z+rad[i])
            maxPosz = mpi.allreduce(maxPosz,mpi.MAX)

            if maxPosz < self.position.z-self.maxParticleRadius:
                particleMass  = 1.0
